/**
 * V131 Reassembler Monitor
 * Monitors reassembler performance and health metrics
 */
...
export class ReassemblerMonitor {
  config: MonitorConfig;
  private history: MonitorMetric[] = [];
  private status: MonitorStatus = 'active';
  private trackedReassemblers: Map<string, Reassembler> = new Map();
  private stats = {
    trackedCount: 0,
    metricCount: 0,
    alertCount: 0,
  ...
  /**
   * Track a reassembler instance
   */
  track(reassembler: Reassembler): boolean {
    if (!reassembler) return false;
    const id = reassembler.getConfig().id;
    this.trackedReassemblers.set(id, reassembler);
    this.stats.trackedCount++;
    this.recordMetric('reassembler.tracked', 1, { reassemblerId: id });
    return true;
  }

  /**
   * Get metrics for a specific reassembler
   */
  getMetrics(reassemblerId?: string): MonitorMetric[] {
    if (reassemblerId) {
      return this.history.filter((m) => m.tags?.reassemblerId === reassemblerId);
    }
    return [...this.history];
  }
  ...
  getStatus(): { status: MonitorStatus; trackedCount: number; metricCount: number } {
    return {
      status: this.status,
      trackedCount: this.trackedReassemblers.size,
  ...
  reset(): void {
    this.history = [];
    this.trackedReassemblers.clear();
    ...
    return JSON.stringify(
      {
        status: this.status,
        config: this.config,
        trackedReassemblers: Array.from(this.trackedReassemblers.keys()),
        stats: this.stats,
        historyLength: this.history.length,
      },
      null,
