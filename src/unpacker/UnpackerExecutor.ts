/**
 * V130 Unpacker Executor
 * Executes unpacker operations and manages execution lifecycle
 */
...
export class UnpackerExecutor {
  ...
  /**
   * Execute an unpacker with the given options
   */
  async execute(unpacker: Unpacker, options: Record<string, unknown> = {}): Promise<ExecutionResult> {
    ...
    if (this.executing.size >= (this.config.maxConcurrent ?? 10)) {
      const result: ExecutionResult = {
        executionId,
        unpackerId: unpacker.getConfig().id,
        success: false,
    ...
    try {
      const result = unpacker.unpack(options);
   ...
   */
  async run(unpackers: Unpacker[], options: Record<string, unknown> = {}): Promise<ExecutionResult[]> {
    const results: ExecutionResult[] = [];
    for (const unpacker of unpackers) {
      const result = await this.execute(unpacker, options);
      results.push(result);
    }
    return results;
  }
